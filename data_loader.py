...
class DataLoader(object):
    ...
    def __init__(self, dataset, path, img_indx, patch_size, patch_num, batch_size=1, istrain=True,
                 filtered_idx=None, test_db=None):
        ...
        # Test transforms
            ...
            if dataset in ['kadid-10k', 'add_kadid-10k'] and len(img_indx) > 20:
                transforms = torchvision.transforms.Compose([
        ...
        if dataset == 'live':
            ...
            self.data = folders.KADID_10kFolder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num,
                filtered_idx=filtered_idx, test_db=test_db)
        elif dataset == 'pipal':
            self.data = folders.PIPALFolder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num, sel_types=[1])
        elif dataset == 'spaq':
